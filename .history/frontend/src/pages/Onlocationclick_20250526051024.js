...
export default function Onbooktickets() {
  ...
  console.log("placeselected in onlocatioclick",placeselected)

  const [selectedSeats, setSelectedSeats] = useState([]);
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [check, setCheck] = useState([]);
  const [all, setAll] = useState([]);
  const day = useMemo(()=>{return ["SUN", "MON", "TUE", "WED", "THUR", "FRI", "SAT"]},[])
        ...
        <div className="seat-info">
          ...
          <div className="legend">
            <div className="legend-item">
              <span style={{backgroundColor:'rgb(101, 101, 239)'}} className="legend-box selected"></span> Selected
            </div>
            <div className="legend-item">
