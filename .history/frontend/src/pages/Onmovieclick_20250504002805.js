...
setrating(ele.rating)
  ...
  return (
    <div className="movie-details-container">
      ...
      <div className="video-container">
        ...
        {movie && (
          <div className="movie-info" style={{border:"none"}}>
            <h2 style={{color:'black'}}>{movie.movie_name}</h2>
            <div style={{fontFamily:'cursive',color: "#9b2121"}}>
              ...
              {rated==false ? <button onClick={()=>{user ? navigate('/ratingdetails',{state:{movie:movie, currentLocation}}) : navigate('/login')}}>Rate</button> : <h2 style={{fontSize:'25px',padding:'0px',margin:'auto auto'}}>Your rating:{rating}/5</h2>}
            </div>
            <h4>
      ...
      <div style={{fontSize:'20px',color:'black',fontFamily:'cursive'}}>{movie.description}</div>
      {/* Cast Section */}
