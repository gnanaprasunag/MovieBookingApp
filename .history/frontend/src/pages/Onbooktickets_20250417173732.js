        ...
        timeplaceFiltered.map((ele, index) => (
          <div key={index} className="timeplace-card">
            <h3 className="place-name">{ele.place}  <button style={{textSize:'100px',backgroundColor:'white',paddingLeft:'30px'}} onClick={() => {setShowModal(true)
              click1(ele.location)
            }}><FontAwesomeIcon icon={faMapMarkerAlt} style={{ color: "red", fontSize: "24px" }} /></button> {alllocations.includes(ele.location) && <span style={{color:'blue'}} className="place-name">visited</span>}</h3>
