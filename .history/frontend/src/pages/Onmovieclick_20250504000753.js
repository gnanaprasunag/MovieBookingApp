...
setrating(ele.rating)
  ...
  return (
    <div className="movie-details-container">
      ...
      <div className="video-container">
        ...
        {movie && (
          <div className="movie-info" style={{border:"none"}}>
            ...
            <div>
              <button style={{backgroundColor:"blue"}} onClick={()=>{user ? navigate('/ratings',{state:{movie:movie}}) : navigate('/login')}}>&#11088;</button>
              {count>0 && <h2 style={{border:"solid",margin:"auto auto",backgroundColor:"blue"}}>{Math.ceil(ratingcount/count)+ "/"+count }</h2>}
              {rated==false ? <button style={{backgroundColor:"blue"}} onClick={()=>{user ? navigate('/ratingdetails',{state:{movie:movie, currentLocation}}) : navigate('/login')}}>Rate</button> : <h2 style={{paddingLeft:'5px',maxWidth:'100px',padding:'0px',backgroundColor:"blue",border:'solid'}}>Your rating:{rating}/5</h2>}
            </div>
            <h4>
      ...
      <div style={{fontSize:'20px',color:'black'}}>{movie.description}</div>
      {/* Cast Section */}
