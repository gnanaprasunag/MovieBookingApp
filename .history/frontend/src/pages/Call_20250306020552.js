...
const CallInitiator = () => {
  // Prefilled registered mobile number (replace with actual registered number)
  const { user } = useSelector((state) => {
    return state.user;
  });

 
  const [phoneNumber, setPhoneNumber] = useState(); // Removing +91 prefix for editing
  ...
  const handleCallInitiation = async () => {
    ...
    try {
      const response = await axios.post('/api/makeCall', {
        to: `+91${phoneNumber}`, // Adding +91 prefix
      });
