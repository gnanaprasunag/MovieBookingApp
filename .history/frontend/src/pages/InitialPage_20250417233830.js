...
export default function App(){
    let appImage="https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcTm_kVL6tVIiffNjveHQUOrfO6SlmUki7TwAC-YjLob48QWEFaT"
...
console.log("screen.width",window.screen.width)
  ...
  <div className="image-container">
    <img src={appImage} alt="App Logo" style={{ width: '400px',backgroundSize:'cover' ,borderRadius:'50px'}} />
  </div>
  ...
  <div className="actions-container">
    ...
    <div className="search-container">
      <input
        ...
        name="search"
        placeholder="Search..."
        style={{width:'500px',border:'solid',marginLeft:'30px'}}
      />
    ...
    <div className="voice-search">
      <button
        ...
        style={{width:'100px',height:'50px',border:'solid',marginRight:'300px',border:'none',paddingRight:'60px'}}
      >
    ...
    <div className="user-info" style={{marginLeft:'20px',border:'solid'}}>
      {localStorage.getItem('token') ? (
        <div style={{marginTop:'30px',border:'solid'}}>
          <h3>{user && `Hi, ${user.firstname}`}</h3>
          <FontAwesomeIcon icon={faUser} style={{ fontSize: '60px', color: 'green' }} />
        </div>
      ) : (
        <div style={{marginTop:'60px'}}><button style={{backgroundColor:'pink',border:'solid'}} onClick={() => navigate('/login')}>Sign In</button></div>
      )}
    </div>
    </div>
