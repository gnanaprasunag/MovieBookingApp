  ...
  return (
    <div className="movie-details-container">
      ...
      <div className="video-container">
        ...
        {movie && (
          <div className="movie-info" style={{border:'solid'}}>
            <h2>{movie.movie_name}</h2>
            <div>
              <button onClick={()=>{user ? navigate('/ratings',{state:{movie:movie}}) : navigate('/login')}} style={{fontSize:"20px"}}>&#11088;</button>
              {count>0 && <h2 style={{paddingLeft:'38px',paddingRight:'100px',maxWidth:'100px'}}>{Math.ceil(ratingcount/count)+ "/"+count }</h2>}
              {rated==false ? <button onClick={()=>{user ? navigate('/ratingdetails',{state:{movie:movie, currentLocation}}) : navigate('/login')}}>Rate</button> : <h2 style={{paddingLeft:'5px',maxWidth:'200px',padding:'0px'}}>Your rating:{rating}/5</h2>}
            </div>
            <h4>
