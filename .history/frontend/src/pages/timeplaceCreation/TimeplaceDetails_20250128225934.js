...
export default function Castcrew(){
  ...
  const handleTimeseats=(value)=>{
    console.log("handleTise",value)
    setTime(value)
  }
      
    const handleRemove=async(movieId)=>{
      console.log("movieId in handle remove in timeplacedetails",movieId)
      try{
        const confirm=window.confirm("Are you sure")
      if(confirm){
          const response=await axios.delete(`/api/timeplace/${movieId}`,{ headers: { 'Authorization': localStorage.getItem('token')}})
          console.log("response in axios remove",response.data)
          const newArr=alltimeplaces.filter((ele)=>{
              return ele._id!=response.data._id
              
          })
          console.log("newarr in listusers handleremove",newArr)
          //setTimeplace(newArr)
    ...
    console.log("hey hi")
        ...
        console.log('timeplace Data:', timeplaceData);  // Log the data being sent
        ...
        axios.post('/api/timeplace',timeplaceData
          ...
          .then(response => {
            //setTimeplace([...timeplace, response.data]);
